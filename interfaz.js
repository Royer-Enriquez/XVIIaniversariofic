...
const talks=d.actividades.filter(a=>a.charla),featured=['23-0945','23-1200','24-1030','24-1115','24-1545','25-0945','25-1030','25-1115'];
...
function renderSpeakers(all=false){
 $('#speakers-grid').innerHTML=(all?speakers:speakers.slice(0,featured.length)).map(a=>{
  const n=a.ponente.replace(/^(?:(?:Dr\.|Dra\.|Ing\.|Mg\.|MSc\.|PhD\.|MBA|PMP)\s*)+/,''),letters=a.ponente==='PMO Francia'?'PMO':n.split(/\s+/).slice(0,2).map(x=>x[0]).join(''),h=hora(a.hora),f=a.foto;
  const picture=f?`<img src="assets/${esc(f.archivo)}" width="${f.ancho}" height="${f.alto}" alt="${esc(f.alt)}" loading="lazy" decoding="async">`:'';
  const visual=f?`<div class="speaker-picture${f.encuadre==='conferencia'?' photo-conference':''}"><span class="portrait-fallback" aria-hidden="true">${esc(letters)}</span>${f.encuadre==='conferencia'?`<div class="conference-crop">${picture}</div>`:picture}</div>`:`<div class="speaker-letter" aria-hidden="true">${esc(letters)}</div>`;
 ...
 $('#ver-ponentes').setAttribute('aria-expanded',all);$('#ver-ponentes').textContent=all?'Mostrar selección de ponentes −':`Ver los ${talks.length} participantes +`;
}
