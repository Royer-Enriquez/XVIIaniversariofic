...
function render(){
 ...
 return `<section class="day-group" aria-labelledby="dia-${day.fecha}"><div class="day-label"><h3 id="dia-${day.fecha}">${day.nombre} ${+day.fecha.slice(-2)} de setiembre</h3><span>${esc(day.lugar)}</span></div>${rows.map(a=>{const h=hora(a.hora),s=saved.has(a.id);return `<article class="event-row" id="actividad-${a.id}"><time class="time" datetime="${a.dia}T${a.hora}:00-05:00">${h.h}<small>${h.p}</small></time><div class="event-content"><span class="topic-tag" data-topic="${esc(a.tema)}">${esc(a.tema)}</span><h4>${esc(a.titulo)}</h4>${a.ponente?`<p class="event-speaker">${esc(a.ponente)}</p>`:''}${a.detalle?`<p class="event-detail">${esc(a.detalle)}</p>`:''}<a class="event-calendar" href="${esc(a.calendarioGoogle)}" target="_blank" rel="noopener noreferrer" aria-label="Añadir a Google Calendar: ${esc(a.titulo)} (se abre en una pestaña nueva)">${icon('calendar')}<span>Añadir a Google Calendar</span><span class="calendar-arrow" aria-hidden="true">↗</span></a></div><button class="save-event" data-save="${a.id}" type="button" aria-pressed="${s}" aria-label="${s?'Quitar de':'Guardar en'} mi agenda: ${esc(a.titulo)}" title="${s?'Quitar de':'Guardar en'} mi agenda">${icon('bookmark')}</button></article>`}).join('')}</section>`}).join('');
...
render();renderSpeakers();refreshClock();['#program-controls','#ver-ponentes','#cargar-mapa','#menu-toggle','#copiar-direccion','#agenda-mobile'].forEach(s=>$(s).hidden=false);document.documentElement.classList.add('js-ready');
})();
